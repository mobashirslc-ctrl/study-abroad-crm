...
const syncDashboard = () => {
    ...
    onSnapshot(q, (snap) => {
        ...
        snap.forEach(dSnap => {
            ...
            if (currentStatus === 'student paid to uni') { final += comm; } 
            else if (currentStatus === 'verified') { pending += comm; }
        }); 
        ...
        allDocs.forEach(d => {
            ...
            html += `<tr><td><b>${d.studentName}</b><br><small>${d.university}</small></td><td>${d.passportNo}</td><td><span class="status-pill ${d.status}">${(d.status || 'submitted').toUpperCase()}</span></td><td><a href="${d.docs?.academic || '#'}" target="_blank" style="color:var(--gold); font-weight:bold;">View</a></td><td>${dateStr}</td></tr>`;
        }); 
...
// --- ৪. সাবমিট অ্যাপ্লিকেশন ও ফাইল আপলোড (FIXED Logic) ---
const uploadFile = async (file) => {
    if (!file) return "No File"; // Undefined jeno na hoy
...
document.getElementById('submitAppBtn').onclick = async () => {
    ...
    try {
        ...
        // Success hobar por notun tab open korbe
        slipWin = window.open('', '_blank');
        ...
        if(slipWin) {
            slipWin.document.write(`
                <html><head><title>Submission Slip</title><style>body { font-family: 'Segoe UI', sans-serif; padding: 40px; background: #f4f7f6; } .container { background: #fff; border: 2px solid #2b0054; border-radius: 12px; padding: 30px; max-width: 600px; margin: auto; } .header { display: flex; justify-content: space-between; border-bottom: 2px solid #eee; padding-bottom: 15px; } .footer { display: flex; justify-content: space-between; margin-top: 30px; border-top: 1px solid #eee; padding-top: 20px; } @media print { .no-print { display: none; } }</style></head>
                <body><div class="container"><div class="header"><h2>SCC HUB</h2><div style="background:#2ecc71;color:#fff;padding:5px 15px;border-radius:20px;">SUBMITTED</div></div><div class="content"><p><b>Date:</b> ${dateStr}</p><p><b>Student:</b> ${sName.toUpperCase()}</p><p><b>Passport:</b> ${sPass.toUpperCase()}</p><p><b>University:</b> ${appData.university}</p></div><div class="footer"><div><div style="width:130px;border-top:1px solid #000"></div><p>Authorized Signature</p></div><div style="text-align:center"><img src="${qrImg}"><br><small>Scan to Track</small></div></div><button onclick="window.print()" class="no-print" style="width:100%;padding:10px;margin-top:20px;background:#2b0054;color:#fff;cursor:pointer;">PRINT SLIP</button></div></body></html>
            `);
