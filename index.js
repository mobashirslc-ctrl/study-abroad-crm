...
mongoose.connect(DB_URI).then(() => console.log("IHP CRM: System Secured & Connected")).catch(err => console.log(err));
...
const Partner = mongoose.model('Partner', new mongoose.Schema({
    ...
    walletBalance: { type: Number, default: 0 }, expiryDate: { type: String, default: 'Pending' }
}));
...
const StudentFile = mongoose.model('StudentFile', new mongoose.Schema({
    ...
    university: String, commissionAmount: Number, visaStatus: { type: String, default: 'Pending' },
    complianceMember: { type: String, default: 'In Review' }, appliedDate: { type: Date, default: Date.now }
}));
...
app.post('/api/admin/add-university', async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: "University Saved Successfully!" });
    } catch (e) { res.status(500).json({ success: false }); }
});

app.patch('/api/admin/update-file-status', async (req, res) => {
    try {
...
app.post('/api/partner/apply', async (req, res) => {
    try {
        ...
        const newFile = new StudentFile({
            partnerId, studentName, contact, university: uni.uniName,
            commissionAmount: Number(uni.partnerCommission)
        });
        await newFile.save();
