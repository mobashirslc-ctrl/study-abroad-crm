...
// স্ট্যাটিক ফাইলের জন্য সঠিক পাথ সেট করা
...
app.get('/admin-panel', async (req, res) => {
  ...
  try {
    ...
    let rows = data.map(s => `<tr><td style="border:1px solid #ddd;padding:10px">${s.name}</td><td style="border:1px solid #ddd;padding:10px"><a href="${s.documentUrl}" target="_blank">View File</a></td></tr>`).join('');
    res.send(`<div style="padding:20px; font-family: sans-serif;"><h2>Admin Panel</h2><table style="width:100%; border-collapse:collapse;">${rows}</table></div>`);
